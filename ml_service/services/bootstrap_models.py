...
class BootstrapTimesFMWrapper(torch.nn.Module):
    ...
    def _load_backbone(self) -> torch.nn.Module:
        ...
        if self.loader_kind == "timesfm2_5":
            try:
            ...
            model = TimesFm2_5ModelForPrediction.from_pretrained(str(backbone_path))
        else:
            from transformers import TimesFmModelForPrediction  # type: ignore

            model = TimesFmModelForPrediction.from_pretrained(str(backbone_path))
